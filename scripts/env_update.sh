...
# Initialize Conda for scripting
# This should be done before most conda commands in a script.
echo "Initializing Conda shell..."
eval "$(conda shell.bash hook)"
if [ $? -ne 0 ]; then echo "ERROR: Failed to initialize Conda shell. Exiting."; exit 1; fi

# --- Environment Creation and Setup ---

# 1. Deactivate any active environment (now that conda shell is initialized)
# This is optional if you are removing the env by name right after, but good practice.
# echo "Deactivating current conda environment if any..."
# conda deactivate # This might still be problematic in some non-interactive setups or if no env is active.
                  # Removing it as conda env remove -n handles the target env regardless.

# 2. Remove the environment if it already exists
echo "Checking for and removing existing Conda environment '$ENV_NAME'..."
if conda env list | grep -q "^${ENV_NAME}[[:space:]]"; then # Regex to match full name
