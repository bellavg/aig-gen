...
def custom_randomized_topological_sort(G: nx.DiGraph, random_generator: random.Random) -> List[int]:
    ...
    for node in list(G.nodes()): # Use list to avoid issues if G is modified
        if node not in in_degree_map:
            ...
            if G.in_degree(node) == 0:
                ...
                warnings.warn(f"Node {node} in G.nodes() but not in initial in_degree_map and has in-degree > 0. Graph state might be inconsistent.")
    ...
    while queue:
        ...
        for v in successors_of_u:
            ...
            if in_degree_map[v] == 0:
                ...
                raise RuntimeError(f"Negative in-degree for node {v} encountered during topological sort. Current order: {result_order}")
    ...
    if processed_nodes_count != G.number_of_nodes():
        ...
        cycle_nodes_or_unreachable = {node for node, degree in in_degree_map.items() if degree > 0 and node in G and node in remaining_nodes}
        ...
        try:
            ...
            raise nx.NetworkXUnfeasible(f"Graph contains a cycle. Example cycle: {cycle}. Processed: {len(result_order)}/{G.number_of_nodes()}. Remaining nodes possibly in cycle: {cycle_nodes_or_unreachable}")
        except nx.NetworkXNoCycle:
            # If no cycle found, it might be disconnected or other issues
            raise nx.NetworkXUnfeasible(f"Topological sort failed. Graph might be disconnected or have other structural issues. Processed: {len(result_order)}/{G.number_of_nodes()}. Remaining nodes: {remaining_nodes}. Nodes with positive in-degree in remainder: {cycle_nodes_or_unreachable}")
...
def _convert_nx_to_pyg_data(graph: nx.DiGraph,
    ...
    try:
        ...
        warnings.warn("Nodes could not be sorted, using arbitrary order. This might lead to inconsistencies if node IDs are not integers.")
    ...
    for old_node_id in node_list: # Iterate in the sorted order
        ...
        try:
            ...
            if not (np.isclose(np.sum(node_type_vec), 1.0) and np.all((np.isclose(node_type_vec, 0.0)) | (np.isclose(node_type_vec, 1.0)))):
                warnings.warn(f"Node {old_node_id} type vector is not one-hot. Sum: {np.sum(node_type_vec)}. Values: {node_type_vec}. Skipping graph.")
                return None
    ...
    for u_old, v_old, attrs in graph.edges(data=True):
        ...
        if u_new is None or v_new is None or not (0 <= u_new < num_nodes_in_graph and 0 <= v_new < num_nodes_in_graph):
            warnings.warn(f"Edge ({u_old}-{v_old}) connects to unknown or out-of-bounds new node IDs ({u_new}, {v_new}). Skipping this edge.")
            continue
        ...
        try:
            ...
            if not (np.isclose(np.sum(edge_type_vec), 1.0) and np.all((np.isclose(edge_type_vec, 0.0)) | (np.isclose(edge_type_vec, 1.0)))):
                warnings.warn(f"Edge ({u_old}-{v_old}) type vector is not one-hot. Sum: {np.sum(edge_type_vec)}. Values: {edge_type_vec}. Skipping graph.")
                return None
            ...
            if not (0 <= edge_channel_index < num_explicit_edge_types):
                warnings.warn(f"Edge ({u_old}-{v_old}) type vector resulted in invalid channel index: {edge_channel_index}. Skipping graph.")
                return None
...
class AIGProcessedAugmentedDataset(InMemoryDataset):
    def __init__(self, root: str, dataset_name: str, split: str,
        ...
        try:
            ...
            raise FileNotFoundError(f"Processed file not found: {self.processed_paths[0]}. Processing might have failed or was skipped.")
        except Exception as e:
    ...
    def process(self):
        ...
        for pkl_file_idx, raw_pkl_path in enumerate(tqdm(self.raw_paths, desc=f"Processing PKL Chunks ({self.split})")):
            ...
            for nx_graph_idx, nx_graph in enumerate(nx_graphs_in_chunk):
                if not isinstance(nx_graph, nx.DiGraph):
                    warnings.warn(f"Item at index {nx_graph_idx} in {raw_pkl_path} is not a NetworkX DiGraph. Skipping.")
                    continue
                    ...
                    if num_actual_nodes > 0 and self.num_augmentations > 0 and self.split == 'train':
                        try:
                            ...
                            for ch_idx in range(NUM_EXPLICIT_EDGE_TYPES):
                                ...
                                adj_channel_for_actual_nodes = base_pyg_data.adj[ch_idx, :num_actual_nodes, :num_actual_nodes]
                                ...
                                for src_node_idx, tgt_node_idx in zip(source_indices.tolist(), target_indices.tolist()):
                                    ...
                                    temp_nx_for_aug.add_edge(tgt_node_idx, src_node_idx) # Corrected: adj is [channel, v, u]
                            ...
                            for aug_idx in range(self.num_augmentations):
                                try:
                                    ...
                                    ordered_node_indices = custom_randomized_topological_sort(temp_nx_for_aug, local_random_generator)
                                    ...
                                    if len(ordered_node_indices) == num_actual_nodes:
                                        ...
                                        permutation_for_padding_nodes = np.arange(num_actual_nodes, MAX_NODES_PAD, dtype=np.int64)

                                        # Combine to get the full permutation array for padded size
                                        full_permutation = np.concatenate([permutation_for_actual_nodes, permutation_for_padding_nodes])
                                        ...
                                        augmented_data_item.adj = augmented_data_item.adj[:, full_permutation_tensor][:, :, full_permutation_tensor]
        ...
        if paths_to_temp_pt_files:
            ...
            try: # Attempt to remove the temporary directory
                ...
                warnings.warn(f"Could not remove temporary directory {temp_individual_graph_pts_dir}: {e_rmdir}. You may need to remove it manually.")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_root', type=str, required=True, help="Root directory for dataset output (e.g., ./datasets_output).")
    parser.add_argument('--dataset_name', type=str, default='aig_pyg_dataset', help="Name for the dataset subfolder within output_root (e.g., my_aig_graphs).")
    parser.add_argument('--file_prefix', type=str, default="real_aigs_part_", help="Prefix of raw PKL files to look for.")
    parser.add_argument('--num_augmentations', type=int, default=5, help="Number of augmentations for each graph in the training set. Set to 0 for no augmentations.")
    parser.add_argument('--num_train_files', type=int, default=4, help="Number of PKL files to allocate for the training set.")
    parser.add_argument('--num_val_files', type=int, default=1, help="Number of PKL files to allocate for the validation set.")
    parser.add_argument('--num_test_files', type=int, default=1, help="Number of PKL files to allocate for the test set.")
    ...
    if args.num_val_files > 0:
        ...
        if take_n > 0:
            ...
            warnings.warn("Not enough files to satisfy requested number of validation files after allocating training, or num_val_files is 0.")
    ...
    if args.num_test_files > 0:
        ...
        if take_n > 0:
            ...
            warnings.warn("Not enough files to satisfy requested number of test files after allocating train/val, or num_test_files is 0.")
    ...
    if train_files_list:
        ...
        del train_dataset; gc.collect() # Release memory if dataset object is not needed further
    else:
    ...
    print(f"Processed data (if any) should be in subdirectories under: {osp.join(args.output_root, args.dataset_name, 'processed')}")
    print("Example: training data at:", osp.join(args.output_root, args.dataset_name, 'processed', 'train_processed_data.pt'))
