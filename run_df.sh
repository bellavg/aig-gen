...
# --- Data Configuration ---
# *** Updated paths to match aigs_pyg/aig/processed/train_... ***
...
# LR_PATIENCE=5           # Removed - No longer used by train script
# SAVE_DIR is now based on the corrected DATASET_NAME ('aig')
SAVE_DIR="${MODEL_NAME}/rand_gen_${DATASET_NAME}_ckpts" # Specific save dir for this run

# --- Validation/Generation Check Parameters ---
# NUM_VAL_GEN=100         # Removed - No longer used by train script
...
# echo " - LR Patience: ${LR_PATIENCE}" # Removed
echo " - Save Directory: ${SAVE_DIR}"
echo "----------------------------------------"

# Pass arguments required by the simplified train_graphs.py
# Removed: --use_lr_scheduler, --lr_scheduler_patience, --validate_generation, --num_val_gen_samples, --temperature_df
srun python -u ${TRAIN_SCRIPT} \
    ...
    `# Add --num_augmentations if your dataset loading needs it` \
    # --num_augmentations 5 \
...
if [ $train_status -ne 0 ]; then    echo "Error: Training script failed with exit code ${train_status}." ; tail -n 50 ./slurm_logs/graphdf_aig_full_${SLURM_JOB_ID}.out ; exit $train_status ; fi
...
# Ensure sample_graphs.py accepts --temperature_df for GraphDF
srun python -u ${GEN_SCRIPT} \
    ...
    --min_nodes ${MIN_NODES} \
    --device ${REQUESTED_DEVICE} \
    `# Pass necessary model config to generation script` \
    `# These MUST match the trained model!` \
    --edge_unroll ${EDGE_UNROLL} \
    # --max_size 64 --node_dim 4 --bond_dim 3 ... (Add if sample_graphs.py needs them)

gen_status=$?
# (Error handling remains the same)
if [ $gen_status -ne 0 ]; then echo "Error: Generation script (${GEN_SCRIPT}) failed with exit code ${gen_status}." ; tail -n 50 ./slurm_logs/graphdf_aig_full_${SLURM_JOB_ID}.out ; exit $gen_status ; fi
...
eval_status=$?
# (Error handling remains the same)
if [ $eval_status -ne 0 ]; then echo "Error: Evaluation script (${EVAL_SCRIPT}) failed with exit code ${eval_status}." ; tail -n 50 ./slurm_logs/graphdf_aig_full_${SLURM_JOB_ID}.out ; exit $eval_status ; fi
