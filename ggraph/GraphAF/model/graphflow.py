...
class GraphFlowModel(nn.Module):
    ...
    def generate(self, temperature=0.75, min_atoms=7, max_atoms=64, disconnection_patience=20):
        """
        ...
        with torch.no_grad():
            ...
            for i in range(max_atoms):
                ...
                graph.add_node(i, type=num2atom[feature_id])
                ...
                for j in range(edge_total):
                    ...
                    while not valid:
                        ...
                        if edge_discrete_id == NO_EDGE_CHANNEL:  # virtual edge
                            ...
                            graph.add_edge(i, j + start, type=num2bond[edge_discrete_id])

                            valid = check_validity(graph)
                            if valid:
                                is_connect = True
                                # print(num2bond_symbol[edge_discrete_id])
                            else:  # backtrack
                                graph.remove_edge(i, j + start)
            ...
            assert final_graph is not None, 'AIG is None...'
            ...
            final_mol = to_directed_aig(final_graph)
