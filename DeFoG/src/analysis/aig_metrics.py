...
import wandb
import numpy as np
from typing import Union, List, Dict, Any, Optional  # Added Optional
import warnings
...
def convert_raw_model_output_to_nx_aig(node_features_tensor: torch.Tensor,
    ...
    """
    Converts raw model output tensors to a canonical NetworkX DiGraph for AIGs.
    """
    nx_graph = nx.DiGraph()
...
class AIGSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule: Any):
        ...
        # Initialize SpectreSamplingMetrics. Its __init__ will call self.loader_to_nx,
        # which will resolve to AIGSamplingMetrics.loader_to_nx.
        # By making split_name optional in AIGSamplingMetrics.loader_to_nx, this call will not error.
        super().__init__(datamodule=datamodule,
                         compute_emd=False,
                         metrics_list=['aig_structural_validity', 'aig_acyclicity', 'degree'])

        # Explicitly (re-)load graphs for AIGSamplingMetrics using its loader_to_nx
        # with specific split names. This will overwrite any graphs loaded by super().__init__.
    ...
    # MODIFIED: Made split_name optional and provided a default value.
    def loader_to_nx(self, loader: Any, split_name: Optional[str] = None) -> List[nx.Graph]:
        """Converts PyG graphs from a dataloader to NetworkX graphs for AIG validation."""
        networkx_graphs: List[nx.Graph] = []

        # Determine the description for tqdm
        desc_prefix = split_name if split_name else "Unknown Split"
        desc_message = f"Loading {desc_prefix} Reference Graphs for Metrics"

        if loader is None or not hasattr(loader, 'dataset') or loader.dataset is None or len(loader.dataset) == 0:
            if self.local_rank == 0:
                warnings.warn(
                    f"AIGSamplingMetrics.loader_to_nx ({desc_prefix}): Loader or its dataset is empty/invalid. Cannot load reference graphs.")
            return networkx_graphs

        for i, pyg_data in enumerate(tqdm(loader.dataset, desc=desc_message,
    ...
    def forward(self, generated_graphs_raw: list, name: str, current_epoch: int, val_counter: int,
        ...
        """
        # ... (rest of the forward method remains the same as in the previous version of aig_metrics_updated) ...
        ...
        to_log.update({
            ...
            'sampling_quality/frac_unique_non_iso_valid_aigs': frac_unique_non_iso_valid,
            'sampling_quality/frac_novel_aigs': frac_non_iso_to_train
        })
        ...
        if ref_metrics:
            ...
            if current_ref_set:
                ratio_keys = [
                    ...
                    'graph_stats/degree_mmd_undirected',
        ...
        if wandb.run and local_rank == 0:
            ...
            wandb.log(wandb_log_data, step=current_epoch, commit=True)
