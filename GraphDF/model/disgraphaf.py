...
class DisGraphAF(nn.Module):
    def __init__(self, mask_node, mask_edge, index_select_edge, num_flow_layer, graph_size,
                 num_node_type, num_edge_type, use_bn=True, num_rgcn_layer=3, nhid=128, nout=128):
        '''
        :param index_nod_edg:
    ...
    def _get_embs_node(self, x, adj):
        """
        Args:
            x: current node feature matrix with shape (batch, N, 9) -> (batch, N, 260)
            adj: current adjacency feature matrix with shape (batch, 4, N, N) -> batch, N, 100, 100
        Returns:
