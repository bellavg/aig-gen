...
class DisGraphAF(nn.Module):
    ...
    def _get_embs(self, x, adj):
        ...
        adj = adj[:, :num_explicit_edges] # (batch, 3, N, N) TODO: check whether we have to use the 4-th slices(virtual bond) or not
        ...
        adj = torch.where(self.mask_edge, adj.unsqueeze(1).repeat(1, self.repeat_num, 1, 1, 1), torch.zeros([1], device=x.device)).view(
            -1, num_explicit_edges, self.graph_size, self.graph_size)  # (batch*repeat_num, 3, N, N)
