...
class DisGraphAF(nn.Module):
    def __init__(self, mask_node, mask_edge, index_select_edge, num_flow_layer, graph_size,
                 num_node_type, num_edge_type, use_bn=True, num_rgcn_layer=3, nhid=128, nout=128, condition_dim=2):
        '''
        Args:
            condition_dim: Dimension of the conditioning signal.
        '''
        ...
        # When conditioning is used, we add the condition dimension to the node feature space.
        if condition_dim > 0:
            self.num_node_type += self.condition_dim
        ...
        self.rgcn = RGCN(
            nfeat=self.num_node_type,
            nhid=nhid,
            nout=nout,
            edge_dim=self.num_edge_type,
    ...
    def forward(self, x, adj, x_deq, adj_deq, num_inputs, num_outputs, tts_condition=None):
        '''
        Args:
            x:        (batch, N, base_node_dim)
            adj:      (batch, edge_dim, N, N)
            num_inputs, num_outputs: conditioning counts (B, 1)
            tts_condition: (batch, N) or (batch, N, cond_dim) truth table condition per node.
        Returns:
            [x_deq, adj_deq]: Updated latent representations.
        '''
        # Pass the tts_condition to _get_embs so that it can be used in conditioning.
        graph_emb_node, graph_node_emb_edge = self._get_embs(x, adj, num_inputs, num_outputs, tts_condition)

        # **NEW: Condition x_deq in the same way as in _get_embs.**
        if tts_condition is not None:
            # Ensure the condition has an explicit channel dimension.
            if tts_condition.dim() == 2:
                tts_condition = tts_condition.unsqueeze(-1)  # (batch, N, 1)
            # Here, we assume x_deq originally has shape (batch, N, base_node_dim)
            # and tts_condition is (batch, N, cond_dim). Concatenate along the last dimension.
            x_deq = torch.cat([x_deq, tts_condition],
                              dim=-1)  # Now x_deq has shape (batch, N, base_node_dim + cond_dim)

        # Iterate through the flow layers.
    ...
    def _get_embs(self, x, adj, num_input_nodes, num_output_nodes, tts_condition=None):
        '''
        Args:
            x: (batch, N, base_node_dim)
            adj: (batch, edge_dim, N, N)
            tts_condition: (batch, N) or (batch, N, cond_dim) (optional)
        Returns:
            graph_emb_node: (batch, N, d)
            graph_node_emb_edge: (batch, (repeat_num-N), 3d)
        '''
        batch_size = x.size(0)
        # Use only the first num_edge_type channels from the adjacency matrix.
        adj = adj[:, :self.num_edge_type]  # (batch, edge_dim, N, N)

        # Expand x according to the mask for autoregressive unrolling.
        x = torch.where(
            self.mask_node,
            x.unsqueeze(1).repeat(1, self.repeat_num, 1, 1),
            torch.zeros([1], device=x.device),
        ).view(-1, self.graph_size, self.num_node_type - self.condition_dim)  # remove condition channels

        # Now, if a truth table condition is provided, append it here.
        if tts_condition is not None:
            # Ensure tts_condition has a channel dimension.
            if tts_condition.dim() == 2:
                tts_condition = tts_condition.unsqueeze(-1)  # shape: (batch, graph_size, 1)
            # Repeat the condition for each unrolling step.
            tts_condition_expanded = tts_condition.repeat_interleave(self.repeat_num, dim=0)
            # Concatenate along the feature dimension.
            x = torch.cat([x, tts_condition_expanded], dim=-1)
        else:
            # Fall back on count-based conditioning if no truth table is provided.
            condition_vector = torch.cat([num_input_nodes, num_output_nodes], dim=-1).float()  # (batch, 2)
            condition_expanded = condition_vector.repeat_interleave(self.repeat_num, dim=0).unsqueeze(
                1)  # (batch*repeat_num, 1, 2)
            x = torch.cat([x, condition_expanded.repeat(1, x.size(1), 1)], dim=-1)

        # Process the adjacency matrix similarly.
        adj = torch.where(
            self.mask_edge,
            adj.unsqueeze(1).repeat(1, self.repeat_num, 1, 1, 1),
            torch.zeros([1], device=x.device)
        ).view(-1, self.num_edge_type, self.graph_size, self.graph_size)

        # Obtain node embeddings from the RGCN.
        node_emb = self.rgcn(x, adj)  # (batch*repeat_num, N, d)
        if hasattr(self, 'batchNorm'):
            node_emb = self.batchNorm(node_emb.transpose(1, 2)).transpose(1, 2)
        node_emb = node_emb.view(batch_size, self.repeat_num, self.graph_size, -1)
        graph_emb = torch.sum(node_emb, dim=2, keepdim=False)  # (batch, repeat_num, d)

        # Split the graph embedding into node-level and edge-level components.
        graph_emb_node = graph_emb[:, :self.graph_size].contiguous()  # (batch, N, d)
        graph_emb_edge = graph_emb[:, self.graph_size:].contiguous().unsqueeze(2)  # (batch, repeat_num-N, 1, d)

        all_node_emb_edge = node_emb[:, self.graph_size:]  # (batch, repeat_num-N, N, d)
        index = self.index_select_edge.view(1, -1, 2, 1).repeat(batch_size, 1, 1, self.emb_size)
...
#         self.num_flow_layer = num_flow_layer
#
#         self.rgcn = RGCN(nfeat=self.num_node_type, nhid=nhid, nout=nout, edge_dim=self.num_edge_type,
#                          num_layers=num_rgcn_layer, dropout=0., normalization=False)
#
#         if use_bn:
#             self.batchNorm = nn.BatchNorm1d(nout)
#
#         #     # Node and edge transformation networks
#         # self.node_st_net = nn.ModuleList([ST_Dis(nout + 2, self.num_node_type, hid_dim=nhid, bias=True)
#         #                                   for _ in range(num_flow_layer)])  # Include +2 for input/output counts
#         # self.edge_st_net = nn.ModuleList([ST_Dis(nout * 3 + 2, self.num_edge_type, hid_dim=nhid, bias=True)
#         #                                   for _ in range(num_flow_layer)])  # Include +2 for input/output counts
#
...
#
#
#     def forward(self, x, adj, x_deq, adj_deq, num_inputs, num_outputs):
#         '''
#         :param x:   (batch, N, 9)
...
#         '''
#         # inputs for RelGCNs
#         graph_emb_node, graph_node_emb_edge = self._get_embs(x, adj, num_inputs, num_outputs)
#
#         for i in range(self.num_flow_layer):
#             # update x_deq
#             node_t = self.node_st_net[i](graph_emb_node).type(x.dtype)
...
#
#
#     def _get_embs(self, x, adj, num_input_nodes, num_output_nodes):
#         '''
...
#
#
#         adj = adj[:, :self.num_edge_type]  # (batch, 2, N, N)
#         x = torch.where(
#             self.mask_node,
#             x.unsqueeze(1).repeat(1, self.repeat_num, 1, 1),
#             torch.zeros([1], device=x.device),
#         ).view(-1, self.graph_size, self.num_node_type-self.condition_dim)
#
#         # Expand condition_vector to match node dimensions
#         condition_vector = torch.cat([num_input_nodes, num_output_nodes], dim=-1).float()  # Shape: (batch, 2)
#         condition_expanded = condition_vector.repeat_interleave(self.repeat_num, dim=0).unsqueeze(
#             1)  # Shape: (batch * repeat_num, 1, 2)
#
#         # Append condition to x
#         x = torch.cat([x, condition_expanded.repeat(1, x.size(1), 1)], dim=-1)  # Shape: (3390, 120, num_node_type + 2)
#
