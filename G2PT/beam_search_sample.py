...
# MODIFIED: Adjusted logic for num_samples=1000 and specific beam configurations.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Sample from a trained AIG model using Beam Search (Normal or Constrained)')
    ...
    parser.add_argument('--num_samples', type=int, default=1000,  # MODIFIED: Default to 1000
                        help='Number of valid samples to generate (or sequences for normal beam search)')
    parser.add_argument('--beam_size', type=int, default=5,
                        help='Number of beams to maintain during search (num_beams). For normal search, this might be overridden or used per sampling call.')
...
def load_model_and_tokenizer(ckpt_path, tokenizer_path, device):
    ...
    try:
        if 'model_args' in checkpoint:
            ...
            cfg_dict = vars(checkpoint['config']) if not isinstance(checkpoint['config'], dict) else checkpoint[
                'config']
            model_args = {'n_layer': cfg_dict.get('n_layer'), 'n_head': cfg_dict.get('n_head'),
            ...
            if None in [model_args['n_layer'], model_args['n_head'], model_args['n_embd'], model_args['block_size'],
                        model_args['vocab_size']]:
                raise ValueError(f"Missing critical model args in checkpoint 'config': {model_args}")
        else:
            raise KeyError("'model_args' or 'config' dictionary/object not found in checkpoint.")
    ...
    try:
        ...
        if load_result.missing_keys: logger.warning(
            f"State dict check (Original Model): Missing keys: {load_result.missing_keys}")
        if load_result.unexpected_keys: logger.warning(
            f"State dict check (Original Model): Unexpected keys: {load_result.unexpected_keys}")
...
def get_token_ids(tokenizer: PreTrainedTokenizerBase) -> Dict[str, Any]:
    ...
    try:
        ...
        if None in [token_ids['NODE_PI_ID'], token_ids['NODE_AND_ID'], token_ids['NODE_CONST0_ID'],
                    token_ids['NODE_PO_ID']]:
...
def generate_normal_beam(
    ...
    # logger.info(f"Starting normal beam search (mode: {search_mode}, diversity: {diversity_mode}, num_beams={beam_size}, num_return={num_return_sequences})...") # Logged by caller
    ...
    if eos_token_id is None:  # If pad_token_id was also None
        logger.error(f"EOS token '{eos_token_str}' not found and no pad_token_id available as fallback.")
        return []
    ...
    use_diversity = num_beam_groups > 1  # diversity_penalty can be 0.0 for group beam search
    if use_diversity and beam_size % num_beam_groups != 0:
        logger.error(
            f"For diverse beam search, num_beams ({beam_size}) must be divisible by num_beam_groups ({num_beam_groups}).")
        return []
    ...
    generation_config = GenerationConfig(
        ...
        num_return_sequences=min(num_return_sequences, beam_size),  # HF behavior
    ...
    try:
        ...
        # logger.info(f"Normal beam search generated {output_ids.shape[0]} sequences.") # Logged by caller
...
if __name__ == '__main__':
    ...
    all_output_token_ids_list = []  # Initialize here for all branches
    effective_beam_size_for_run = args.beam_size  # Default, may be overridden

    if args.search_type == 'normal':
        logger.info("Running NORMAL beam search...")

        if args.do_sample_beam:
            logger.info(
                f"Normal beam search with SAMPLING. Target samples: {args.num_samples}, Beam size per call: {args.beam_size}")
            effective_beam_size_for_run = args.beam_size  # For sampling, this is the k in "top-k sampling from beams"
            num_generated_total = 0
            num_calls = 0
            num_return_per_call = args.beam_size  # Get all sampled beams from each call

            max_calls = math.ceil(
                args.num_samples / num_return_per_call) if num_return_per_call > 0 else args.num_samples
            safety_max_calls = max_calls + 10  # Allow some leeway

            while num_generated_total < args.num_samples:
                num_calls += 1
                if num_calls > safety_max_calls and num_generated_total < args.num_samples:
                    logger.warning(
                        f"Exceeded maximum configured sampling calls ({safety_max_calls}). Generated {num_generated_total}/{args.num_samples}. Stopping.")
                    break

                current_request_num_sequences = min(num_return_per_call, args.num_samples - num_generated_total)
                if current_request_num_sequences <= 0: break  # Should not happen if loop condition is correct

                logger.info(
                    f"Sampling call {num_calls}: Requesting {current_request_num_sequences} sequences (beam_size={args.beam_size}, temp={args.temperature}).")
                with ctx:
                    output_token_ids_batch = generate_normal_beam(
                        model=model,
                        tokenizer=tokenizer,
                        device=device,
                        prompt="<boc>",
                        beam_size=args.beam_size,
                        num_return_sequences=current_request_num_sequences,  # Request what's needed, up to beam_size
                        max_new_tokens=args.max_new_tokens,
                        temperature=args.temperature,
                        do_sample=True,
                        num_beam_groups=1,  # Standard sampling, not diverse
                        diversity_penalty=0.0
                    )
                if output_token_ids_batch:
                    all_output_token_ids_list.extend(output_token_ids_batch)
                    num_generated_total += len(output_token_ids_batch)
                    logger.info(
                        f"Collected {len(output_token_ids_batch)} sequences from call {num_calls}. Total collected: {num_generated_total}/{args.num_samples}")
                else:
                    logger.warning(f"Sampling call {num_calls} returned no sequences. Stopping early if this persists.")
                    if num_calls > max_calls // 2 and not all_output_token_ids_list:  # If many calls yield nothing
                        logger.error("Sampling consistently returning no sequences. Aborting.")
                        break

            if len(all_output_token_ids_list) > args.num_samples:
                all_output_token_ids_list = all_output_token_ids_list[:args.num_samples]
            logger.info(f"Finished sampling. Total sequences from sampling: {len(all_output_token_ids_list)}")

        elif args.num_beam_groups > 1:
            effective_beam_size_for_run = 1000
            _num_beam_groups_for_diverse = args.num_beam_groups
            logger.info(
                f"Normal beam search with DIVERSITY. Target samples: {args.num_samples}. Using effective beam_size={effective_beam_size_for_run}, num_beam_groups={_num_beam_groups_for_diverse}, diversity_penalty={args.diversity_penalty}.")
            if effective_beam_size_for_run % _num_beam_groups_for_diverse != 0:
                logger.warning(
                    f"For diverse beam search, effective_beam_size ({effective_beam_size_for_run}) should ideally be divisible by num_beam_groups ({_num_beam_groups_for_diverse}). Generation might fail or behave unexpectedly.")

            with ctx:
                all_output_token_ids_list = generate_normal_beam(
                    model=model,
                    tokenizer=tokenizer,
                    device=device,
                    prompt="<boc>",
                    beam_size=effective_beam_size_for_run,
                    num_return_sequences=args.num_samples,
                    max_new_tokens=args.max_new_tokens,
                    temperature=args.temperature,  # Temperature still affects scoring
                    do_sample=False,  # Typically False for diverse beam search, user specified sampling separately
                    num_beam_groups=_num_beam_groups_for_diverse,
                    diversity_penalty=args.diversity_penalty
                )
            logger.info(f"Finished diverse beam search. Total sequences: {len(all_output_token_ids_list)}")

        else:  # Standard Beam Search (not sampling, not diverse)
            effective_beam_size_for_run = 1000
            logger.info(
                f"Normal beam search (standard). Target samples: {args.num_samples}. Using effective beam_size={effective_beam_size_for_run}.")
            with ctx:
                all_output_token_ids_list = generate_normal_beam(
                    model=model,
                    tokenizer=tokenizer,
                    ...
                    num_return_sequences=args.num_samples,
                    max_new_tokens=args.max_new_tokens,
                    temperature=args.temperature,  # Temperature affects scoring
                    do_sample=False,
                    num_beam_groups=1,
                    diversity_penalty=0.0
                )
            logger.info(f"Finished standard beam search. Total sequences: {len(all_output_token_ids_list)}")

        generated_sequences_text = tokenizer.batch_decode(all_output_token_ids_list, skip_special_tokens=False)

    elif args.search_type == 'constrained':
        logger.info("Running CONSTRAINED beam search...")
        effective_beam_size_for_run = args.beam_size  # For constrained, it's user-defined beam_size
        ...
        num_generated_ok = 0
        attempts = 0
        # Adjust max_attempts if beam_size is small and num_samples is large
        # Consider that each successful generation reduces remaining samples.
        # Max attempts per sample might be a better metric, e.g., max_attempts_per_sample = max(20, args.beam_size * 4)
        # Total max_attempts = args.num_samples * 5 (simple multiplier)
        max_attempts_factor = max(10,
                                  args.beam_size * 2) if args.beam_size > 0 else 20  # At least 10 attempts per sample goal
        max_total_attempts = args.num_samples * max_attempts_factor
        logger.info(
            f"Constrained search: target {args.num_samples}, beam_size {args.beam_size}, max_total_attempts {max_total_attempts}")

        while num_generated_ok < args.num_samples and attempts < max_total_attempts:
            attempts += 1
            logger.info(
                f"Constrained search attempt {attempts}/{max_total_attempts} for overall sample {num_generated_ok + 1}/{args.num_samples}")
            with ctx:
                beams = generate_constrained_beam(
                    ...
                    device=device,
                    prompt="<boc>",
                    beam_size=args.beam_size,  # User-defined beam_size for constrained
                    max_new_tokens=args.max_new_tokens,
                    temperature=args.temperature,
                )
            if beams:
                # In constrained search, we often take the best one found per attempt if it's good
                # Or, if beam_size > 1, we could try to validate multiple beams from this one call.
                # For now, sticking to one best sequence per successful call.
                best_log_prob, best_sequence_ids = beams[0]  # Top beam
                ...
                if seq_str.endswith("<eog>") and "<bog>" in seq_str and "<eoc>" in seq_str:
                    generated_sequences_text.append(seq_str)  # Store the string directly
                    num_generated_ok += 1
                    logger.info(
                        f"Found valid sequence {num_generated_ok}/{args.num_samples}. LogProb: {best_log_prob:.2f}")
                else:
                    logger.debug(
                        f"Constrained search result (len {len(best_sequence_ids)}) not a complete graph structure. Seq: {seq_str[:100]}...")
            else:
                logger.debug(f"Constrained search attempt {attempts} yielded no valid beams.")

            if attempts % 100 == 0 and num_generated_ok < args.num_samples:  # Progress update
                logger.info(
                    f"Progress after {attempts} attempts: {num_generated_ok}/{args.num_samples} sequences found.")

        if num_generated_ok < args.num_samples:
            logger.warning(
                f"Constrained search finished. Found {num_generated_ok}/{args.num_samples} sequences after {attempts} attempts.")
        else:
            logger.info(
                f"Constrained search completed. Found {num_generated_ok}/{args.num_samples} sequences in {attempts} attempts.")
    ...
    logger.info(f"Finished generation. Total sequences obtained for parsing: {len(generated_sequences_text)}")
    ...
    for i, seq_str in enumerate(generated_sequences_text):
        try:
            ...
            if isinstance(graph, nx.DiGraph):
                ...
                logger.warning(
                    f"seq_to_nxgraph did not return a DiGraph for sequence {i}. Got {type(graph)}. Seq: {seq_str[:100]}...")
    ...
    # For normal search, generated_sequences_text comes from all_output_token_ids_list.
    # For constrained, generated_sequences_text is populated directly.
    ...
    # Use effective_beam_size_for_run for the filename tag
    beam_tag = f"_k{effective_beam_size_for_run}"

    sampling_tag = ""
    if args.search_type == 'normal' and args.do_sample_beam:
        sampling_tag = "_sample"

    diversity_tag = ""
    if args.search_type == 'normal' and args.num_beam_groups > 1 and not args.do_sample_beam:  # Diversity not combined with multi-call sampling in filename
        diversity_tag = f"_diverseG{args.num_beam_groups}P{args.diversity_penalty:.1f}"

    temp_tag = ""
    # Temperature is relevant for constrained, or normal if sampling
    if args.search_type == 'constrained' or (args.search_type == 'normal' and args.do_sample_beam):
        temp_tag = f"_t{args.temperature:.1f}"
    # Also add temp_tag if diverse beam search with non-default temp (though less common for it to be main factor)
    elif args.search_type == 'normal' and args.num_beam_groups > 1 and args.temperature != 1.0:  # Default HF temp is 1.0
        temp_tag = f"_t{args.temperature:.1f}"
    ...
    logger.info(f"Effective beam size for run (used for filename tag _k): {effective_beam_size_for_run}")
