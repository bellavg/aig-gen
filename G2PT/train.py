...
# System settings
#device = 'cpu'  # Device for training; options: 'cpu', 'cuda', etc.
...
# --- DDP Setup (or single-process setup) ---
if ddp:
    init_process_group(backend=backend)
    ddp_rank = int(os.environ['RANK'])
    ddp_local_rank = int(os.environ['LOCAL_RANK'])
    ddp_world_size = int(os.environ['WORLD_SIZE'])
    device = f'cuda:{ddp_local_rank}' # Set device for DDP process
    ...
    if torch.cuda.is_available():
        device = 'cuda'
        print("CUDA available. Using device: cuda")
    elif hasattr(torch.backends, 'mps') and torch.backends.mps.is_available():
         device = 'mps'
         print("MPS available. Using device: mps")
    else:
         device = 'cpu'
         print("CUDA/MPS not available. Using device: cpu")
...
# --- Dtype and Autocast Setup (Done ONCE) ---
if 'cuda' in device: # Check if final device is CUDA-based
...
print(f"Autocast context type: {device_type}, ptdtype: {ptdtype}")
...
num_loader_workers = 8 if ddp else 0 # Use 8 workers if DDP is on, 0 otherwise
